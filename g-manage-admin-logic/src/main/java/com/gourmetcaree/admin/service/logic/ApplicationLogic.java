...
public class ApplicationLogic extends AbstractAdminLogic {
	 ...
	 */
	public List<TApplication> doSearchApplication(PageNavigateHelper pageNavi, int targetPage, Map<String, String> map) throws WNoResultException, ParseException {
		List<TApplication> entityList = null;
		...
		try {
			...
			entityList = jdbcManager
							.selectBySql(TApplication.class, sb.toString(), params.toArray())
							.disallowNoResult()
							.getResultList();
	 ...
	 * @throws ParseException
	 */
	public List<TObservateApplication> doSearchObservateApplication(PageNavigateHelper pageNavi, int targetPage, Map<String, String> map) throws WNoResultException, ParseException {
		...
		try {
			StringBuilder sb = new StringBuilder("");
			List<Object> params = new ArrayList<Object>();
			createObservateSearchSql(sb, params, map);
...
//			entityList = getSearchResultList(where, pageNavi);
			entityList = jdbcManager
							.selectBySql(TObservateApplication.class, sb.toString(), params.toArray())
							.disallowNoResult()
							.getResultList();
	 ...
	 * @throws WNoResultException
	 */
	public List<TApplication> getApplicationList(Map<String, String> map, String sortKey) throws ParseException, WNoResultException {
		List<TApplication> entityList = null;
		try {
			StringBuilder sb = new StringBuilder("");
			List<Object> params = new ArrayList<Object>();
...
//			entityList = getSearchResultList(where, pageNavi);
	 ...
	 * @throws WNoResultException
	 */
	public List<TObservateApplication> getObservateApplicationList(Map<String, String> map, String sortKey) throws ParseException, WNoResultException {
		List<TObservateApplication> entityList = null;
		try {
			StringBuilder sb = new StringBuilder("");
			List<Object> params = new ArrayList<Object>();
...
//			entityList = getSearchResultList(where, pageNavi);
	...
	private void createApplicationSearchSql(StringBuilder sb, List<Object> params, Map<String, String> map ) throws ParseException {
		...
		if (StringUtils.isNotBlank(map.get("lowerAge"))) {
			sb.append(" AND  (");
			sb.append("       ( APP.age >= ? )");
			sb.append("    OR ( date_part('year',age(current_date, APP.birthday)) >= ? )");
			sb.append(" ) ");
		...
		if (StringUtils.isNotBlank(map.get("memberFlg"))) {
			sb.append(" AND APP.member_flg = ? ");
			params.add(NumberUtils.toInt(map.get("memberFlg")));
		}
		...
		if (StringUtils.isNotBlank(map.get("nameKana"))) {
            ...
            sb.append(SqlUtils.createLikeSearch(
            		nameKana,
                    params,
                    " REPLACE(REPLACE(APP.name_kana, '　',''), ' ','') LIKE ? "));
            sb.append(" ) ");
		}

		// 店舗応募区分が指定されている場合
		if (StringUtils.isNotBlank(map.get("observationKbn"))) {
			sb.append(" AND APP.observation_kbn = ? ");
			params.add(NumberUtils.toInt(map.get("observationKbn")));
		}
	 ...
	 * @throws ParseException
	 */
	private void createObservateSearchSql(StringBuilder sb, List<Object> params, Map<String, String> map ) throws ParseException {
		sb.append(" SELECT ");
		sb.append("     TOA.* ");
		sb.append(" FROM ");
		sb.append("    t_observate_application TOA");
		sb.append(" INNER JOIN ");
		sb.append("    t_web WEB");
		sb.append(" ON TOA.web_id = WEB.id ");
		sb.append(" INNER JOIN ");
		sb.append("    m_customer CUS");
		sb.append(" ON TOA.customer_id = CUS.id ");
		sb.append(" WHERE 1=1");

		if(!StringUtils.isBlank(map.get("areaCd"))) {
			sb.append(" AND TOA.area_cd = ? ");
			params.add(NumberUtils.toInt(map.get("areaCd")));
		}

		// 応募IDが指定されている場合
		if (StringUtils.isNotBlank(map.get("id"))) {
			sb.append(" AND TOA.id = ? ");
			params.add(NumberUtils.toInt(map.get("id")));
		}

		// webIdが指定されている場合
		if (StringUtils.isNotBlank(map.get("webId"))) {
			sb.append(" AND TOA.web_id = ? ");
			params.add(NumberUtils.toInt(map.get("webId")));
		}
		...
		if (StringUtils.isNotBlank(map.get("lowerAge"))) {
			sb.append(" AND  (");
			sb.append("       ( TOA.age >= ? )");
			sb.append("    OR ( date_part('year',age(current_date, TOA.birthday)) >= ? )");
			sb.append(" ) ");
			params.add(NumberUtils.toInt(map.get("lowerAge")));
			params.add(NumberUtils.toInt(map.get("lowerAge")));
		}
		...
		// 性別が指定されている場合
		if (StringUtils.isNotBlank(map.get("sexKbn"))) {
			sb.append(" AND TOA.sex_kbn = ? ");
			params.add(NumberUtils.toInt(map.get("sexKbn")));
		}

		// 会員フラグが指定されている場合
		if (StringUtils.isNotBlank(map.get("memberFlg"))) {
			sb.append(" AND TOA.member_flg = ? ");
			params.add(NumberUtils.toInt(map.get("memberFlg")));
		}

		// 端末区分が指定されている場合
		if (StringUtils.isNotBlank(map.get("terminalKbn"))) {
			sb.append(" AND TOA.terminal_kbn = ? ");
			params.add(NumberUtils.toInt(map.get("terminalKbn")));
		}
		...
		// 店舗応募区分が指定されている場合
		if (StringUtils.isNotBlank(map.get("observationKbn"))) {
			sb.append(" AND TOA.observation_kbn = ? ");
			params.add(NumberUtils.toInt(map.get("observationKbn")));
		}
		...
		if (StringUtils.isNotBlank(map.get("applicationStartDate"))) {
			...
			sb.append(" AND TOA.application_datetime >= ? ");
			params.add(new Timestamp(DateUtils.formatDate(map.get("applicationStartDate"), map.get("applicationStartHour"), map.get("applicationStartMinute")).getTime()));
