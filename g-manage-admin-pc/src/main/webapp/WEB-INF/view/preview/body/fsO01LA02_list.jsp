...
<c:if test="${existDataFlg}">
	<c:set var="pathFormat" value="${common['gc.arbeitPreview.smartPhone.url.format']}" scope="page" />
	<c:forEach items="${relationDtoList}" var="dto">
		<%-- パスを生成 --%>
		<c:choose>
			<c:when test="${imgMethodKbn eq ENUM_IMG_METHOD_KBN_SESSION }">
				<c:set var="DETAIL_MSG_PATH" value="${gf:makePathConcat2Arg('/shopListPreview/smartDetail/indexSession', dto.id, inputFormKbn)}" scope="page" />
			</c:when>
			<c:otherwise>
				<c:set var="DETAIL_MSG_PATH" value="${gf:makePathConcat2Arg('/shopListPreview/smartDetail', dto.id, webId)}" scope="page" />
			</c:otherwise>
		</c:choose>

		<div class="shop_data_wrapper">
			<div class="shop_data_detail">
				<a href="${f:url(f:h(DETAIL_MSG_PATH))}">
