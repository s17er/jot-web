...
<c:set var="SHUTOKEN_AREA_CD" value="<%=MAreaConstants.AreaCd.SHUTOKEN_AREA%>" scope="page" />
<c:set var="SENDAI_AREA_CD" value="<%=MAreaConstants.AreaCd.SENDAI_AREA%>" scope="page" />
...
<gt:typeList name="dbDetailAreaKbnShutokenList" typeCd="<%=MTypeConstants.ShutokenDetailAreaKbn.TYPE_CD %>"  />
<gt:typeList name="dbDetailAreaKbnSendaiList" typeCd="<%=MTypeConstants.SendaiDetailAreaKbn.TYPE_CD %>"  />
...
<gt:allArea name="allAreaList"/>
	...
	<c:if test="${existDataFlg eq true}">
		...
		<div id="wrapper">
			<c:forEach items="${dataList}" var="dto" varStatus="preSts">
				<c:if test="${preSts.first eq false}">
				...
				<table cellpadding="0" cellspacing="1" class="preview_table" >
							...
							<c:forEach items="${dto.detailAreaKbnList}" var="t" varStatus="sts">
								<c:choose>
									<c:when test="${areaCd eq SHUTOKEN_AREA_CD }">
										${f:label(t, dbDetailAreaKbnShutokenList, 'value', 'label')}
									</c:when>
									<c:otherwise>
										${f:label(t, dbDetailAreaKbnSendaiList, 'value', 'label')}
									</c:otherwise>
								</c:choose>
								<c:if test="${sts.last eq false}">
									/
					...
					<tr>
						...
						<%--
						<td colspan="4">
							<c:forEach items="${dto.otherConditionKbnList}" var="t" varStatus="sts">
								${f:label(t, dbOtherConditionKbnList, 'value', 'label')}
								<c:if test="${sts.last eq false}">
									/
								</c:if>
							</c:forEach>
					...
					<tr>
						<th>IP電話</th>
						<c:choose>
							<c:when test="${fn:length(dto.ipPhoneList) gt 0}">
								<c:set var="tdStyle" value="" />
							</c:when>
							<c:otherwise>
								<c:set var="tdStyle" value="style=\"background-color: #C33;\"" />
							</c:otherwise>
						</c:choose>
						<td colspan="2" ${tdStyle}>
							<c:if test="${fn:length(dto.ipPhoneList) gt 0}">
								<c:forEach items="${dto.ipPhoneList}" var="t">
									<span style="font-size: 130%;">${f:h(t)}</span><br />
								</c:forEach>
							</c:if>
						</td>
						<th>受信元番号</th>
						<td colspan="2">
							<c:if test="${fn:length(dto.customerTelList) gt 0}">
								<c:forEach items="${dto.customerTelList}" var="t">
									<span style="font-size: 130%;">${f:h(t)}</span><br />
								</c:forEach>
							</c:if>
						</td>
						<th>原稿表示<br />電話番号</th>
						<td colspan="3">
							<c:if test="${fn:length(dto.telList) gt 0}">
				...
				<c:when test="${imgMethodKbn eq ENUM_IMG_METHOD_KBN_SESSION }">
					<c:set var="DETAIL_PREVIEW_PATH" value="${gf:makePathConcat2Arg('/detailPreview/detail/index', dto.id, inputFormKbn)}" scope="page" />
					<c:set var="MAIN_1_IMG_PATH" value="${gf:makePathConcat2Arg('/util/imageUtility/displayWebdataImage', MATERIAL_KBN_MAIN_1, dto.idForDirName)}" scope="page" />
				...
				<c:otherwise>
					<c:set var="DETAIL_PREVIEW_PATH" value="${gf:makePathConcat1Arg('/detailPreview/dbDetail/index', dto.id)}" scope="page" />
				...
				<div id="contents">
					...
					<div id="header_icon" class="clear">
						...
						<ul class="oubo clear">
							...
							<%--
							<c:if test="${dto.applicationFormKbn eq APPLICATION_FORM_KBN_EXIST or dto.applicationFormKbn eq APPLICATION_FORM_KBN_LONG}">
								<li><a href="#" title="応募フォーム"><img src="${f:h(frontHttpUrl)}${f:h(imgLocation)}/btn_form.gif" width="95" height="25" alt="応募フォーム" /></a></li>
							</c:if>
							--%>
							...
							<c:if test="${not empty dto.movieUrl}">
								<c:choose>
									<c:when test="${imgMethodKbn eq ENUM_IMG_METHOD_KBN_SESSION }">
										<c:set var="MOVIE_PATH" value="/detailPreview/detail/displayMovie/${f:h(dto.id)}/${f:h(inputFormKbn)}" scope="page" />
									</c:when>
									<c:otherwise>
										<c:set var="MOVIE_PATH" value="/detailPreview/dbDetail/displayMovie/${f:h(dto.id)}" scope="page" />
									</c:otherwise>
								</c:choose>
