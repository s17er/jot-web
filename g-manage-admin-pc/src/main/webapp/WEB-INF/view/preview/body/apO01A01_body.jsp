...
<!-- メイン画像表示切替用設定-ココまで -->
		...
		<div id="contents">
			<div class="intro clear">
				...
				<div class="photo_area">
					<c:choose>
						<c:when test="${sizeKbnABC}">
							<c:if test="${imageCheckMap[MATERIAL_KBN_MAIN1]}">
								<img src="${f:url(MAIN1_IMG_PATH)}" width="330" height="250" alt="${f:h(manuscriptName)}" />
							</c:if>
						</c:when>
						...
						<div id="fade">
							...
							<c:if test="${imageCheckMap[MATERIAL_KBN_MAIN2]}">
									<img src="${f:url(MAIN2_IMG_PATH)}" width="330" height="250" alt="${f:h(manuscriptName)}" />
							</c:if>
							<c:if test="${imageCheckMap[MATERIAL_KBN_MAIN3]}">
									<img src="${f:url(MAIN3_IMG_PATH)}" width="330" height="250" alt="${f:h(manuscriptName)}" />
							</c:if>
						</div>
					...
					<c:if test="${sizeKbnDE eq true}">
						<ul class="sub_photo clear">
							<c:if test="${imageCheckMap[MATERIAL_KBN_MAIN1]}">
								<li><img src="${f:url(MAIN1_IMG_PATH)}" width="102" height="77" alt="${f:h(manuscriptName)}" /></li>
							</c:if>
							<c:if test="${imageCheckMap[MATERIAL_KBN_MAIN2]}">
								<li><img src="${f:url(MAIN2_IMG_PATH)}" width="102" height="77" alt="${f:h(manuscriptName)}" /></li>
							</c:if>
							<c:if test="${imageCheckMap[MATERIAL_KBN_MAIN3]}">
								<li><img src="${f:url(MAIN3_IMG_PATH)}" width="102" height="77" alt="${f:h(manuscriptName)}" /></li>
							</c:if>
						</ul>
					</c:if>
				</div>
				...
				<c:if test="${blockRight eq true and blockRightExist eq true}">
					<div class="intro2 clear">
						<c:if test="${blockPhoto eq true and imageCheckMap[MATERIAL_KBN_RIGHT]}">
							<div class="right_photo">
								<img src="${f:url(RIGHT_IMG_PATH)}" width="180" height="250" alt="${f:h(manuscriptName)}" />
							</div>
						</c:if>
					...
					<div class="cap_area">
						<ul class="cap_photo clear">
							<c:if test="${imageCheckMap[MATERIAL_KBN_PHOTO_A]}">
								<li>
									<img src="${f:url(PHOTO_A_IMG_PATH)}" width="235" height="170" alt="${f:h(manuscriptName)}" />
							...
							<c:if test="${imageCheckMap[MATERIAL_KBN_PHOTO_B]}">
								<li>
									<img src="${f:url(PHOTO_B_IMG_PATH)}" width="235" height="170" alt="${f:h(manuscriptName)}" />
							...
							<c:if test="${imageCheckMap[MATERIAL_KBN_PHOTO_C]}">
								<li>
									<img src="${f:url(PHOTO_C_IMG_PATH)}" width="235" height="170" alt="${f:h(manuscriptName)}" />
				...
				<c:if test="${blockAttentionHere and blockAttentionHereExist and not empty attentionHereTitle}">
					<div class="chumoku">
						<div class="ch_area clear">
							...
							<div class="ch_photo">
								<img src="${f:url(ATTENTION_HERE_IMG_PATH)}" width="175" height="150" alt="${f:h(manuscriptName)}" />
							</div>
							</c:if>
