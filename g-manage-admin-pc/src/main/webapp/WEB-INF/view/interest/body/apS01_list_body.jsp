...
<gt:typeList name="dbEmployPtnKbnList" typeCd="<%=MTypeConstants.EmployPtnKbn.TYPE_CD %>"/>
...
// 「DatePicker」の搭載
	$(function(){
		$("#interestStartDate").datepicker();
		$("#interestEndDate").datepicker();
	});
			...
			<table cellpadding="0" cellspacing="1" border="0" class="search_table">
				...
				<tr>
					...
					<c:choose>
						<c:when test="${userDto.authLevel eq AUTH_LEVEL_ADMIN}">
							<th>氏名</th>
							<td class="release"><html:text property="where_memberName" size="40" /></td>
						</c:when>
						<c:otherwise>
							<td colspan="3"></td>
						</c:otherwise>
                ...
                <tr>
                    ...
                    <td class="release">
                        <div id="companyAjax">
                            <c:choose>
                                <c:when test="${userDto.authLevel eq AUTH_LEVEL_ADMIN or userDto.authLevel eq AUTH_LEVEL_STAFF}">
                ...
                <tr>
                    ...
                    <td class="">
						<html:select property="where_hopeJob">
							...
							<html:optionsCollection name="dbJobKbnList"/>
						</html:select>
                    </td>
					<th>希望業態</th>
					<td class="release">
				...
				<c:if test="${userDto.authLevel eq AUTH_LEVEL_ADMIN}">
					<tr>
						<th>メールアドレス</th>
						<td class="release"><html:text property="where_loginId" size="40" styleClass="disabled" /></td>
						<th>メールアドレス(携帯)</th>
						<td class="release"><html:text property="where_subMailAddress" size="40" styleClass="disabled" /></td>
					</tr>
				</c:if>
				<tr>
					<th >気になる通知先名(原稿名)</th>
					<td class="release"><html:text property="where_manuscriptName" size="40" /></td>
					<th>原稿番号</th>
				...
				<tr>
					...
					<td colspan="3">
						<html:select property="where_empPtnKbn"  >
							<html:option value="">--</html:option>
							<html:optionsCollection name="dbEmployPtnKbnList"/>
						</html:select>
					</td>
				</tr>
				<tr>
					...
					<td colspan="3" class="release">
						<html:text property="where_interestStartDate" size="10" styleId="interestStartDate" styleClass="disabled" maxlength="10" />&nbsp;
						<html:text property="where_interestStartHour" size="2" styleClass="disabled" maxlength="2" />：
						<html:text property="where_interestStartMinute" size="2" styleClass="disabled" maxlength="2" />&nbsp;～&nbsp;
						<html:text property="where_interestEndDate" size="10" styleId="interestEndDate" styleClass="disabled" maxlength="10" />&nbsp;
						<html:text property="where_interestEndHour" size="2" styleClass="disabled" maxlength="2" />：
						<html:text property="where_interestEndMinute" size="2" styleClass="disabled" maxlength="2" />
					</td>
				</tr>
			</table>
			<hr />
	...
	<!-- #wrap_result# -->
		...
		<table cellpadding="0" cellspacing="0" border="0" class="page">
			...
			<c:if test="${dto.linkFlg eq true}">
				...
				<c:set var="pageLinkPath" scope="page" value="/interest/list/changePage/${dto.pageNum}" />
		...
		<table cellpadding="0" cellspacing="0" border="0" class="cmn_table list_table">
			<tr>
				...
				<th width="50" class="posi_center bdrd_bottom">気になるID</th>
				<th width="35" class="posi_center bdrd_bottom">会員番号</th>
				<th class="bdrd_bottom" colspan="2">原稿名</th>
				<th width="110" class="bdrd_bottom">希望業態</th>
				<th width="110" class="bdrd_bottom posi_center">希望雇用形態</th>
				<th width="110" class="bdrd_bottom bdrs_right">希望職種</th>
			</tr>
			<tr>
				...
				<th>氏名</th>
				<th>住所</th>
				<th width="110">気になる日時</th>
				<th width="70" class="posi_center">エリア</th>
				<th width="30" class="posi_center bdrs_right">詳細</th>
			</tr>

			<c:forEach var="dto" items="${interestList}" varStatus="status">
			...
			<c:set var="classStr" value="${gf:odd(status.index, 'blue', '')}" scope="page" />
				<tr>
					...
					<td class="posi_center bdrd_bottom ${classStr}">
						<c:choose>
							<c:when test="${dto.memberId == 0 }">--&nbsp;</c:when>
							<c:when test="${dto.memberId != 0 }">${f:h(dto.memberId)}&nbsp;</c:when>
						</c:choose>
					</td>
					<td class="bdrd_bottom ${classStr}" colspan="2">${f:h(dto.manuscriptName)}&nbsp;</td>
					<td class="bdrd_bottom posi_center ${classStr}">
						<c:if test="${dto.hopeIndustryKbnList.size() == 0 || dto.memberId == 0}">--</c:if>
						<c:forEach var="t" items="${dto.hopeIndustryKbnList}" varStatus="status">
							<c:if test="${status.first eq false}">
								&nbsp;/&nbsp;
							</c:if>
							${f:label(t, dbIndustryList, 'value', 'label')}
						</c:forEach>
					<td width="110" class="bdrd_bottom posi_center ${classStr}">
						<c:if test="${dto.hopeEmployPtnKbnList.size() == 0 || dto.memberId == 0}">--</c:if>
						<c:forEach var="t" items="${dto.hopeEmployPtnKbnList}" varStatus="status">
							<c:if test="${status.first eq false}">
								&nbsp;/&nbsp;
							</c:if>
							${f:label(t, dbEmployPtnKbnList, 'value', 'label')}
						</c:forEach>
					</td>
					<td width="110" class="bdrd_bottom posi_center ${classStr}">
						<c:if test="${dto.hopeJobKbnList.size() == 0 || dto.memberId == 0}">--</c:if>
						<c:forEach var="t" items="${dto.hopeJobKbnList}" varStatus="status">
							<c:if test="${status.first eq false}">
								&nbsp;/&nbsp;
							</c:if>
				...
				<tr>
					...
					<td class="posi_center ${classStr}">
						<c:choose>
							<c:when test="${dto.memberId == 0 }">--&nbsp;</c:when>
							<c:when test="${dto.memberId != 0 }">${f:h(dto.age)}&nbsp;</c:when>
						</c:choose>
					</td>
					<td class="${classStr}"><c:if test="${userDto.authLevel eq AUTH_LEVEL_ADMIN}">${f:h(dto.memberName)}&nbsp;</c:if>&nbsp;</td>
					<td class="${classStr}">${f:label(dto.prefecturesCd, prefecturesList, 'value', 'label')}${f:h(dto.municipality)}&nbsp;</td>
					<td class="${classStr}">${f:h(dto.interestDateTime)}</td>
					<td class="posi_center ${classStr}">${f:label(dto.areaCd, areaList, 'value', 'label')}&nbsp;</td>
					<td class="posi_center bdrs_right ${classStr}">
						<c:choose>
							<c:when test="${dto.memberId == 0 }">--&nbsp;</c:when>
							<c:when test="${dto.memberId != 0 }"><a href="${f:url(dto.detailPath)}">詳細</a></c:when>
						</c:choose>
					</td>
				</tr>
			</c:forEach>
