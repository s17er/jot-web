<%@page pageEncoding="UTF-8"%>
<tiles:insert template="/WEB-INF/view/common/adminLayout.jsp" flush="false">
	<tiles:put name="title" value="${SITE_NAME} | 号数データ削除確認"/>
	<tiles:put name="content" value="/WEB-INF/view/maintenance/body/apJ02_conf_body.jsp" />

	<% // 以下ページ切り替え用パラメータ %>
	<tiles:put name="pageKbn" value="${PAGE_DELETE}" />
	<tiles:put name="actionPath" value="/volume/delete/" />
	<tiles:put name="pageTitle1" value="号数データ削除確認" />
	<tiles:put name="pageTitleId1" value="title_volumeDelConf" />
	<tiles:put name="navigationText1" value="管理画面トップへ" />
	<tiles:put name="navigationPath1" value="/top/menu/" />
	<tiles:put name="navigationText2" value="メンテナンスメニューへ" />
	<tiles:put name="navigationPath2" value="/maintenance/menu/" />
	<tiles:put name="navigationText3" value="号数データ一覧へ" />
	<tiles:put name="navigationPath3" value="/volume/list/" />
	<tiles:put name="defaultMsg" value="以下の内容でよろしければ、「削除」ボタンから削除して下さい。" />
</tiles:insert>