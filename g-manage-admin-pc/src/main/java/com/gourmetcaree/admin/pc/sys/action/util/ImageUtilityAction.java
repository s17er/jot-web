...
import com.gourmetcaree.admin.pc.sys.form.util.ImageUtilityForm;
...
public class ImageUtilityAction extends PcAdminAction {
	 ...
	 * ファイルをセッション用のフォルダから取得します。
	 * @return
	 */
	@Execute(validator = false, urlPattern = "displayWebdataImage/{materialKbn}/{webId}")
	@MethodAccess(accessCode="UTIL_IMAGEUTILITY_DISPLAYWEBDATAIMAGE")
	public String displayWebdataImage() {
	...
	protected void handleImage() {
		...
		dirName.append(imageUtilityForm.webId);
		dirName.append("_");
		dirName.append(session.getId());

		MaterialDto dto = null;
		try {
			dto = WebdataFileUtils.getWebdataImageFile(dirPath, dirName.toString(), imageUtilityForm.materialKbn);
			//ストリームに出力
			outputToStream(dto);
	 ...
	 * @param binArray
	 */
	private void outputToStream (MaterialDto dto) {
