 ...
 * @version 1.0
 */
public class ApplicationService extends AbstractGroumetCareeBasicService<TApplication> {
	 ...
	 */
	public TApplication getApplicationDataById(int id) throws WNoResultException {

		try {
			TApplication entity = jdbcManager.from(TApplication.class)
	 ...
	 */
	public List<TApplication> doSearch(PageNavigateHelper pageNavi,Map<String, String> map) throws WNoResultException, ParseException {
	...
	public int getSearchCount(Where where) {
		...
		Long count = jdbcManager.from(TApplication.class)
						.leftOuterJoin("mCustomer" , "mCustomer.deleteFlg = ?", DeleteFlgKbn.NOT_DELETED)
						.where(where)
						.disallowNoResult()
						.getCount();
	 ...
	 * @return 応募エンティティリスト
	 */
	public List<TApplication> getSearchResultList(Where where, PageNavigateHelper pageNavi) {

		List<TApplication> entityList = jdbcManager.from(TApplication.class)
										.leftOuterJoin("mCustomer" , "mCustomer.deleteFlg = ?", DeleteFlgKbn.NOT_DELETED)
										.where(where)
	 ...
	 * @throws WNoResultException
	 */
	public List<TApplication> getSearchResultListForCsv(Where where, String sortKey) {
	...
	public String createJoinedApplicantName(List<Integer> applicationIdList, final String delimiter) {
		...
		where.in(WztStringUtil.toCamelCase(TApplication.ID), applicationIdList);

		jdbcManager.from(TApplication.class)
				.where(where)
				.orderBy(TApplication.ID)
				.iterate(new IterationCallback<TApplication, Void>() {

					@Override
					public Void iterate(TApplication entity, IterationContext context) {
						if (entity == null) {
							return null;
						}
	...
	public long countByWebId(Integer webId) {
		...
		SimpleWhere where = new SimpleWhere();
		where.eq(WztStringUtil.toCamelCase(TApplication.WEB_ID), webId);
