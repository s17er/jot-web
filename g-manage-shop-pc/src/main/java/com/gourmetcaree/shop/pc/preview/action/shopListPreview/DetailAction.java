...
import com.gourmetcaree.shop.pc.preview.form.shopListPreview.DetailForm;
...
public class DetailAction extends PcShopAction {
	 ...
	 * @return
	 */
	@Execute(validator = false, reset = "resetForm", input = TransitionConstants.Preview.JSP_APO03A01)
	public String index() {
		return show();
	}
	 ...
	 * @return
	 */
	@Execute(validator = false, reset = "resetForm", urlPattern = "changeShop/{id}", input = TransitionConstants.Preview.JSP_APO03A01)
	...
	private void createCustomerData() {
		MCustomer entity = customerService.findById(userDto.getCustomerCd());
	...
	private void createShopListData() {
		try {
			...
			if (StringUtils.isBlank(detailForm.id)) {
				// 最初に並ぶ店舗エンティティ
				TShopList firstEntity = shopListService.getFirstShopListByCustomerId(userDto.getCustomerCd());
			...
			try {
				...
				for (RelationShopListRetDto retDto : retDtoList) {
					...
					dto.detailPath = GourmetCareeUtil.makePath("/shopListPreview/detail/", "changeShop", String.valueOf(retDto.id));
				...
				// 路線取得
				detailForm.routeList = shopListRouteService.findByShopListId(Integer.parseInt(detailForm.id));
