...
public class InputAction extends MailListAction {
	 ...
	 * @return
	 */
	@Execute(validator = false, reset = "resetForm", input = TransitionConstants.Application.JSP_SPC02C01)
	public String index() {
	 ...
	 * @return
	 */
	@Execute(validator = false, reset = "resetForLumpSend", input = TransitionConstants.Application.JSP_SPC02C01)
	...
	private String show() {
		...
		return TransitionConstants.Application.JSP_SPC02C01;
	}
	 ...
	 * @return
	 */
	@Execute(validator = true, input = TransitionConstants.Application.JSP_SPC02C01)
	public String conf() {
		...
		return TransitionConstants.Application.JSP_SPC02C02;
	}
	...
	public String back() {
		...
		if (ApplicationMailForm.FROM_MAIL_DETAIL.equals(inputForm.fromPageKbn)) {
			return   String.format("/applicationMail/detail/indexAgain/%s%s", inputForm.originalMailId[0], GourmetCareeConstants.REDIRECT_STR);
		} else {
			return "/application/list/showList".concat(GourmetCareeConstants.REDIRECT_STR);
		}
	}
	 ...
	 * @return
	 */
	@Execute(validator = false, input = TransitionConstants.Application.JSP_SPC02C01)
	...
	private void sendMail() {
		...
		for (String originalMailId :inputForm.originalMailId) {
			...
			try {
				applicantName = sendMailLogic.getApplicantNameByMailId(mailId);
			} catch (WNoResultException e) {
			...
			try {
				...
				sendMailLogic.doReplyApplicationMail(property, mailPattern);
			} catch (WNoResultException e) {
				//対象が存在しない場合は楽観的排他制御のエラーを返す
				throw new SOptimisticLockException();
			}
