...
import com.gourmetcaree.shop.pc.application.form.applicationMail.DetailForm;
...
public class DetailAction extends MailListAction {
	...
	public String fromApplicationName;
	 ...
	 * @return
	 */
	@Execute(validator = false, reset = "resetForm", urlPattern = "index/{applicationId}", input = TransitionConstants.Application.JSP_SPC02R01)
	public String index() {
//
//		//必要なパラメータをチェック
	 ...
	 * @return
	 */
	@Execute(validator = false, urlPattern = "indexAgain/{applicationId}", input = TransitionConstants.Application.JSP_SPC02R01)
	 ...
	 * @return
	 */
	@Execute(validator = false, reset = "resetForm", urlPattern = "indexFromDetailList/{id}/{applicationId}", input = TransitionConstants.Application.JSP_SPC02R01)
	...
	private String show() {
		...
		Integer applicationId = detailForm.applicationId;

		try {
			application = applicationLogic.findByIdFromApplication(applicationId);
			application.age = GourmetCareeUtil.convertToAge(application.birthday);
			application.tApplicationAttributeList = applicationAttributeService.getApplicationAttrListByApplicationId(application.id);
			if (application.shopListId != null) {
				TShopList tShopList = shopListService.findByIdAllowNoResult(application.shopListId);
				if (tShopList != null){
					fromApplicationName = tShopList.shopName;
				} else {
					// 未削除から店舗情報が見つからない場合は削除済み含め取得
					fromApplicationName = shopListService.findByIdIncludeDelete(application.shopListId).shopName + "（店舗データ無し）";
				}
			} else {
				fromApplicationName = customerService.findById(application.customerId).customerName;
			}

			for(TApplicationAttribute applicationAttribute : application.tApplicationAttributeList) {
			...
			detailForm.qualificationKbnList = GourmetCareeUtil.toIntToStringArray(
					applicationAttributeService.getApplicationAttrValue(
							application.id, MTypeConstants.QualificationKbn.TYPE_CD));

			detailForm.applicationSchoolHistory = applicationSchoolHistoryService.getApplicationSchoolHistoryByApplicationId(application.id);

			List<TApplicationCareerHistory> entityList = applicationCareerHistoryService.getApplicationCareerHistoryByApplicationId(application.id);
			...
			for(MailSelectDto mailDto : applicationLogic.getApplicationIdEachMail(property, applicationId, MAIL_KBN.APPLICATION_MAIL).retList) {
				...
				if(mailDto.sendKbn == MTypeConstants.SendKbn.RECEIVE
					...
					applicationLogic.changeMailToOpened(mailDto.id);
				}
			}
		...
		if(application.memberId != null) {
			try {
				memberService.findById(application.memberId);
		...
		return TransitionConstants.Application.JSP_SPC02R01;
	}
	 ...
	 * @return
	 */
	@Execute(validator = false, input = TransitionConstants.Application.JSP_SPC02R01)
	public String submit() {
		...
		if(detailForm.mailBody.isEmpty() || detailForm.subject.isEmpty()) {
			...
			return "/applicationMail/detail/indexAgain/" + applicationId;
		}
		...
		return "/applicationMail/detail/index/" + applicationId +"?redirect=true";
	}
	...
	private void sendMail(Integer originaMailId) {
		...
		try {
			applicantName = sendMailLogic.getApplicantNameByMailId(originaMailId);
		} catch (WNoResultException e) {
		...
		try {
			sendMailLogic.doReplyApplicationMail(property, MailPattern.NORMAL_REPLY);
		} catch (WNoResultException e) {
			throw new SOptimisticLockException();
		}
	}
	 ...
	 * @return
	 */
	@Execute(validator = false, urlPattern = "delete/{id}/{deleteVersion}", input = TransitionConstants.Application.JSP_SPC02R01)
	 ...
	 * @return
	 */
	@Execute(validator = false, removeActionForm = true, input = TransitionConstants.Application.JSP_SPC02R01)
	public String comp() {
		return TransitionConstants.Application.JSP_SPC02D03;
	}
