...
<table cellpadding="0" cellspacing="0" border="0" class="cmn_table detail_table">
	<tr>
		<th width="150">宛先</th>
		<td>
			<c:forEach var="m" items="${arbeitApplicationId}" varStatus="status" >
				<c:set var="name" value="${fromName[status.index]}" scope="page" />
				<c:set var="url" value="${f:url(gf:makePathConcat1Arg( '/arbeit/detail/subApplicationDetail',m))}" scope="page" />

				<a href="#" onclick="detailWindow('${url}', '${f:h(m)}')">
				${f:h(name)}&nbsp;
				</a>
...
<div class="wrap_btn">
<html:hidden property="hiddenRandomKey" value="${hiddenRandomKey}"/>
