...
<div id="wrap_mail">
	...
	<div class="menu_tab">
		<div class="menu_list">
			<ul>
				...
				<li class="tab_active">
					<a href="${f:url('/arbeitMail/list/')}">グルメdeバイト応募メール
						<c:if test="${unReadArbeitMailFlg}">
						<span id="mail_notification"></span>
						</c:if>
					</a>
				</li>
	...
	<div class="send_receive_tab">
		<ul class="send_receive_ul">
			<li class="send_receive_list send <c:if test='${sendKbn == 2}'>on</c:if>"><a href="${f:url('/arbeitMail/list/')}" class="send_receive_anc">受信</a></li>
			<li class="send_receive_list receive <c:if test='${sendKbn == 1}'>on</c:if>"><a href="${f:url('/arbeitMail/list/sendBox')}" class="send_receive_anc">送信</a></li>
		</ul>
	</div>
							...
							<tbody>
								<c:forEach var="dto" items="${dataList}">
									<tr>
										...
										<td class="table-name"><p><a href="${f:url(gf:concat2Str('/arbeitMail/detail/index/', dto.arbeitApplicationId)) }?fromPage=receive"><span class="name">${dto.fromName }</span><span class="age">（${dto.age}）</span></a></p></td>
										<td class="table-status">
											<ul class="table-status">
												<li id="status">
													<div class="selectbox">
														<select name="selectionKbn" id="selectionKbn${dto.arbeitApplicationId}" onchange="onchangeSelectionKbn('selectionKbn${dto.arbeitApplicationId}', '${dto.arbeitApplicationId}');">
												...
												<li id="memo">
													<div class="mest_memo">
													<html:text property="memo" value="${f:h(dto.memo)}" placeholder="メモを入力" id="memo${dto.arbeitApplicationId}" class="form-bg"/>
													<input type="button" name="editMemo" value="登録" class="btn_entry" onClick="onchangeMemo('memo${dto.arbeitApplicationId}', '${dto.arbeitApplicationId}');">
													</div>
												</li>
											</ul>
										</td>
			...
			<table cellpadding="0" cellspacing="0" border="0" class="all_table"><tbody>
				...
				<c:forEach var="dto" items="${dataList}" varStatus="status">
					<tr>
						...
						<td><a href="${f:url(gf:concat2Str('/arbeitMail/detail/index/', dto.arbeitApplicationId)) }?fromPage=send">${f:h(dto.subject)}</a>&nbsp;</td>
