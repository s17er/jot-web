...
<div id="wrap_mail">
	...
	<p class="explanation">
		過去1年間のメール履歴一覧です。<br>
		応募者をクリックすると、メール内容・応募者の詳細がご覧いただけます。
	</p>
	<div class="menu_tab">
		<div class="menu_list">
			<ul>
	...
	<div class="send_receive_tab">
		<ul class="send_receive_ul">
			<li class="send_receive_list send <c:if test='${sendKbn == 2}'>on</c:if>"><a href="${f:url('/applicationMail/list/')}" class="send_receive_anc">受信</a></li>
			<li class="send_receive_list receive <c:if test='${sendKbn == 1}'>on</c:if>"><a href="${f:url('/applicationMail/list/sendBox')}" class="send_receive_anc">送信</a></li>
		</ul>
	</div>
					...
					<div class="narrowing_area">
						...
						<div class="narrowing_check">
							...
							<div class="narr_occu">
								<span>職種</span><br>
								<div class="selectbox">
									<html:select property="where_jobKbn" class="occupation_bar" styleId="selectionKbn">
										<html:optionsCollection name="recruitedJobKbnList" />
									</html:select>
								</div>
							</div>
							<div class="narr_empl">
								<span>雇用形態</span><br>
								<div class="selectbox">
									<html:select property="where_employPtnKbn" class="employment_bar" styleId="selectionKbn">
										<html:optionsCollection name="recruitedEmployPtnList" />
									</html:select>
								</div>
							</div>
							<div class="narr_stat">
							...
							<tbody>
								<c:forEach var="dto" items="${dataList}">
									<tr>
										...
										<td class="table-name"><p><a href="${f:url(gf:concat2Str('/applicationMail/detail/index/', dto.applicationId)) }?fromPage=receive"><span class="name">${dto.fromName }</span><span class="age">（${dto.age}）</span></a></p></td>
										<td class="table-status">
											<ul class="table-status">
												<li id="status">
													<div class="selectbox">
														<select name="selectionKbn" id="selectionKbn${dto.applicationId}" onchange="onchangeSelectionKbn('selectionKbn${dto.applicationId}', '${dto.applicationId}');">
												...
												<li id="memo">
													<div class="mest_memo">
													<html:text property="memo" value="${f:h(dto.memo)}" placeholder="メモを入力" id="memo${dto.applicationId}" class="form-bg"/>
													<input type="button" name="editMemo" value="登録" class="btn_entry" onClick="onchangeMemo('memo${dto.applicationId}', '${dto.applicationId}');">
													</div>
												</li>
											</ul>
										</td>
									...
									<tr style="height: 110px;">
										<td class="table-info" style="flex-wrap: wrap;">
											...
											<div class="repo">
												<ul class="">
													<c:choose>
														...
														<c:otherwise>
															<li>${dto.areaName}</li>
															<li>${f:label(dto.jobKbn, recruitedJobKbnList, 'value', 'label')}</li>
															<li>${f:label(dto.employPtnKbn, recruitedEmployPtnList, 'value', 'label')}</li>
															<li>飲食経験年数 : ${f:label(dto.foodExpKbn, foogExpKbnList, 'value', 'label')}</li>
														</c:otherwise>
													</c:choose>
												</ul>
											</div>
											<div style="flex-basis: 100%; content: '';"></div>
											<span style="color: #aaa;">応募遷移　${ dto.fromApplicationName }</span>
										</td>
										<c:choose>
						...
						<div class="btn_page">
							<div class="btn">
								<div class="wrap_btn">
									...
									<html:submit property="outputCsv" value="ユーザー情報のCSV出力" styleId="btn_csv" class="sp_none"/>
								</div>
							</div>
							<div class="page">
								<p><!--
			...
			<table cellpadding="0" cellspacing="0" border="0" class="all_table"><tbody>
				...
				<c:forEach var="dto" items="${dataList}" varStatus="status">
					<tr>
						...
						<td><a href="${f:url(gf:concat2Str('/applicationMail/detail/index/', dto.applicationId)) }?fromPage=send">${f:h(dto.subject)}</a>&nbsp;</td>
