		...
		<div class="page_back">
			<c:choose>
				<c:when test="${fromPage == 'send' }">
					<a href="${f:url('/observateApplicationMail/list/sendBox')}" id="btn_back"><span class="pc_none">質問メール一覧へ</span>戻る</a>
				</c:when>
				<c:otherwise>
					<a href="${f:url('/observateApplicationMail/list/showList')}" id="btn_back"><span class="pc_none">質問メール一覧へ</span>戻る</a>
				</c:otherwise>
			</c:choose>
		</div>
		<h2 class="name">${observateApplication.name }（${observateApplication.age}）<span class="hope_txt"></span>
		</h2>
		<div class="menu_tab">
			<div class="menu_list">
			<ul>
		...
		<div class="made_mest">
			<div class="made_status">
				<div class="selectbox">
					...
					<select class="status_bar" name="selectionKbn" id="selectionKbn" onchange="onchangeSelectionKbn('selectionKbn', '${observateApplication.id }');">
							<c:forEach items="${selectionList}" var="i">
								<c:set var="selected" value="${(i.value eq observateApplication.selectionFlg) ? 'selected' : ''}" />
			...
			<div class="made_memo">
				<html:text property="memo" value="${f:h(observateApplication.memo)}" placeholder="メモを入力" id="memo${observateApplication.id}" class="form-bg"/>
				<input type="button" name="editMemo" value="登録" class="btn_entry" onClick="onchangeMemo('memo${observateApplication.id }', '${observateApplication.id }');">
			</div>
		</div>
			...
			<div class="tab_area">
				<div class="tab_contents tab_active" id="detail_Mail">
					...
					<c:if test="${!unsubscribeFlg}">
						...
						<html:hidden property="observateApplicationId"/>
						...
						<div class="det_profile">
							...
							<div class="r_details">
								<table cellpadding="0" cellspacing="0" border="0" class="detail_table">
									<tbody><tr>
										<th>質問日時</th>
										<td><fmt:formatDate value="${observateApplication.applicationDatetime}" pattern="yyyy/MM/dd HH:mm" /></td>
									</tr>
									<tr>
										<th>質問ID</th>
										<td>${f:h(observateApplication.id)} &nbsp;</td>
									</tr>
									<tr>
										<th>氏名(漢字）</th>
										<td>${f:h(observateApplication.name)} &nbsp;</td>
									</tr>
									<tr>
										<th>氏名(カナ）</th>
										<td>${f:h(observateApplication.nameKana)} &nbsp;</td>
									</tr>
									<tr>
										<th>性別</th>
										<td>${f:label(observateApplication.sexKbn, sexList, 'value', 'label')} &nbsp;</td>
									</tr>
									<tr>
										<th>年齢</th>
										<td><fmt:formatDate value="${observateApplication.birthday}" pattern="yyyy/MM/dd" />&nbsp;&nbsp;${f:h(observateApplication.age)}才&nbsp;</td>
									</tr>
									<tr>
										<th>電話番号</th><td>${f:h(observateApplication.phoneNo1)}-${f:h(observateApplication.phoneNo2)}-${f:h(observateApplication.phoneNo3)}&nbsp;</td>
									</tr>
									<tr>
										<th>メールアドレス</th>
										<td>${f:h(observateApplication.pcMail)} &nbsp;</td>
									</tr>
									<tr>
										<th>内容</th>
										<td>${f:br(f:h(observateApplication.contents))} &nbsp;</td>
									</tr>
								</tbody></table>
							</div>
						</div>
