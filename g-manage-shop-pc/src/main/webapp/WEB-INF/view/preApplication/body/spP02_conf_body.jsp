...
//<![CDATA[
	...
	var onchangeSelectionKbn = function(selectId, id) {
		...
		$.ajax({
			url: '${f:url("/preApplicationMail/list/ajaxSelectionFlg/")}',
			data: selectionJson,
			type: 'post'
		});
	...
	var onchangeMemo = function(selectId, id) {
		...
		$.ajax({
			url: '${f:url("/preApplicationMail/list/editMemo/")}',
			data: selectionJson,
			type: 'post'
		});
	};
		...
		<div class="page_back">
			<c:choose>
				<c:when test="${fromPage == 'send' }">
					<a href="${f:url('/preApplicationMail/list/sendBox')}" id="btn_back"><span class="pc_none">プレ応募メール一覧へ</span>戻る</a>
				</c:when>
				<c:otherwise>
					<a href="${f:url('/preApplicationMail/list/showList')}" id="btn_back"><span class="pc_none">プレ応募メール一覧へ</span>戻る</a>
				</c:otherwise>
			</c:choose>

		</div>
		<h2 class="name">${preApplication.memberId }（${preApplication.age}）<span class="hope_txt">希望職種：【${employPtnMap[preApplication.employPtnKbn]}】${f:label(preApplication.jobKbn, jobKbnList, 'value', 'label')}</span>
		</h2>
		<div class="menu_tab">
			<div class="menu_list">
			<ul>
		...
		<div class="made_mest">
			<div class="made_status">
				<div class="selectbox">
					...
					<select class="status_bar" name="selectionKbn" id="selectionKbn" onchange="onchangeSelectionKbn('selectionKbn', '${preApplication.id }');">
							<c:forEach items="${selectionList}" var="i">
								<c:set var="selected" value="${(i.value eq preApplication.selectionFlg) ? 'selected' : ''}" />
			...
			<div class="made_memo">
				<input type="text" value="${f:h(preApplication.memo)}" name="memo" id="memo${preApplication.id}" placeholder="メモを入力">
				<input type="button" name="editMemo" value="登録" class="btn_entry" onClick="onchangeMemo('memo${preApplication.id }', '${preApplication.id }');">
			</div>
		</div>
						...
						<div class="det_profile">
							...
							<div class="r_details">
								<table cellpadding="0" cellspacing="0" border="0" class="detail_table">
									<tbody><tr>
										<th>応募日時</th>
										<td><fmt:formatDate value="${preApplication.applicationDatetime}" pattern="yyyy/MM/dd HH:mm" /></td>
									</tr>
									<tr>
										<th>応募ID</th>
										<td>${f:h(preApplication.id)} &nbsp;</td>
									</tr>
									<tr>
										<th>性別</th>
										<td>${f:label(preApplication.sexKbn, sexList, 'value', 'label')} &nbsp;</td>
									</tr>
									<tr>
										<th>年齢</th>
										<td>${f:h(preApplication.age)}才&nbsp;</td>
									</tr>
									<tr>
										<th>住所</th>
										<td>
										<gt:prefecturesList name="prefecturesList"  />
										${f:label(preApplication.prefecturesCd, prefecturesList, 'value', 'label')}
										${f:h(preApplication.municipality)}
										&nbsp;</td>
									</tr>
									<tr>
										<th>希望雇用形態</th>
										<td>${f:label(preApplication.employPtnKbn, employPtnKbnList, 'value', 'label')} &nbsp;</td>
									</tr>
									<tr>
										<th>希望職種</th>
										<td>${f:label(preApplication.jobKbn, jobKbnList, 'value', 'label')} &nbsp;</td>
									</tr>
									<tr>
										<th>希望業態･店舗など</th>
										<td>${f:h(preApplication.hopeIndustry)} &nbsp;</td>
									</tr>
									<tr>
										<th>希望連絡方法･時間帯</th>
										<td>${f:h(preApplication.connectTime)} &nbsp;</td>
									</tr>
									<tr>
										<th>その他の希望連絡方法</th>
										<td>${f:h(preApplication.contactMethod)} &nbsp;</td>
									</tr>
									<tr>
										<th class="bdrs_bottom">希望面接日</th>
										<td class="bdrs_bottom">第一希望：${f:h(preApplication.firstDesired)}&nbsp;<br>
											第二希望：${f:h(preApplication.secondDesired)}&nbsp;<br>
											第三希望：${f:h(preApplication.thirdDesired)}&nbsp;<br>
										</td>
									</tr>
									<tr>
										<th>その他</th>
										<td>${f:h(preApplication.note)} &nbsp;</td>
									</tr>
								</tbody></table>
							</div>
						</div>
						<div class="det_pr">
							...
							<div class="r_details">
								<table cellpadding="0" cellspacing="0" border="0" class="detail_table">
									<tbody><tr>
										<th>飲食業界経験年数</th>
										<td>${f:label(preApplication.foodExpKbn, foogExpKbnList, 'value', 'label')}&nbsp;</td>
									</tr>
									<tr>
										<th class="bdrs_bottom">マネジメント経験</th>
										<td class="bdrs_bottom">役職：${f:label(preApplication.expManagerKbn, expManagerKbnList, 'value', 'label')}&nbsp;<br>
											経験年数：${f:label(preApplication.expManagerYearKbn, expManagerYearKbnList, 'value', 'label')}&nbsp;<br>
											人数：${f:label(preApplication.expManagerPersonsKbn, expManagerPersonsKbnList, 'value', 'label')}&nbsp;<br>
										</td>
									</tr>
									<tr>
										<th>海外勤務経験</th>
										<td>${f:label(preApplication.foreignWorkFlg, foreignWorkFlgKbnList, 'value', 'label')}</td>
									</tr>
									<tr>
										<th>取得資格</th>
										<td>
									...
									<tr>
										<th>自己PR</th>
										<td>${f:br(f:h(preApplication.applicationSelfPr))}</td>
									</tr>
								</tbody></table>
							</div>
						</div>
