...
<table cellpadding="0" cellspacing="0" border="0" class="cmn_table list_table stripe_table">
	...
	<tr>
		...
		<td><a href="${f:url(gf:makePathConcat2Arg('/preApplicationMail/detail/indexFromDetailList', dto.id, dto.applicationId)) }">${f:h(dto.subject)}</a>&nbsp;</td>
...
<div class="wrap_btn">
	<c:choose>
		<c:when test="${hideHeaderFlg eq true}">
			<html:button property="" value="　"  onclick="location.href='${f:url('/preApplicationMail/list/showList/')}'" styleId="btn_back" />
		</c:when>
		<c:when test="${empty applicationId}">
			<html:button property="" value="　"  onclick="location.href='${f:url('/preApplication/list/showList/')}'" styleId="btn_back" />
		</c:when>
	</c:choose>
</div>
</div>
