	...
	<c:forEach items="${dataList}" var="dto">
		...
		<c:choose>
			<c:when test="${previewMethodKbn eq ENUM_DRAFT_PREVIEW }">
				<c:set var="DETAIL_PREVIEW_PATH" value="${gf:makePathConcat3Arg('/detailPreview/mobileDraftDetail/index', dto.id, accessCd, areaCd)}" scope="page" />
			</c:when>
			<c:otherwise>
				<c:set var="DETAIL_PREVIEW_PATH" value="${gf:makePathConcat1Arg('/detailPreview/mobileDetail/index', dto.id)}" scope="page" />
			</c:otherwise>
		</c:choose>
